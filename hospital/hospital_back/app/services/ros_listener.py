...
支持新消息格式：{medicine_id}_{prescription_code}_{status}
"""
import asyncio
import json
import socket
...
_ros_state: Dict[str, Any] = {
    ...
    "current_medicine_id": None,        # 当前处理的药品ID
    "current_step": 1,
    "steps": [
...
_audio_state: Dict[str, Any] = {
    ...
    "current_prescription_code": None,  # 当前单子的处方编码
}
...
def parse_ros_message(data: str) -> Dict[str, Any]:
    ...
    3. 新格式（药品坐标发送）: "{medicine_id}_{prescription_code}_{status}"
       例如："1_012026070300122_running-started"
            "1_012026070300122_running-step5-return"
    4. 处方完成格式: "{prescription_code}_all_completed"
       例如："012026070300122_all_completed"
    5. 任务结束格式: "{prescription_code}_end"
       例如："012026070300122_end"
    ...
    # ===== 新增：解析药品坐标发送格式 {medicine_id}_{prescription_code}_{status} =====
    # 格式：1_012026070300122_running-started
    #      1_012026070300122_running-step5-return
    # 分割为3部分：medicine_id, prescription_code, status
    parts = data.split("_")
    if len(parts) >= 3:
        # 尝试将第一部分转换为整数（药品ID）
        try:
            ...
            status = "_".join(parts[2:])  # 状态部分可能包含下划线（如 running-step5-return）
            return {
                "status": status,
            ...
            # 第一部分不是整数，说明是旧格式 {prescription_code}_{status}
            pass
    
    # ===== 解析旧格式 {prescription_code}_{status} =====
    # 格式：012026070300122_all_completed
    #      012026070300122_end
    #      012026070300122_running-started
    if len(parts) == 2:
        return {
            "status": parts[1],
...
def update_prescription_workflow_db(prescription_code: str, status: str, medicine_id: Optional[int] = None) -> bool:
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # ===== 修复：移除HIS处方状态更新逻辑 =====
            # 原逻辑：收到 end 就更新HIS处方为 dispensed（导致第二个药品start发不出去）
            # 新逻辑：HIS状态更新移到 ros_websocket_listener 的 end 处理中
            #         仅当最后一个药品完成时才更新（见修改点5）
        ...
        try:
            ...
            try:
                async with websockets.connect(ws_url) as ws:
                    ...
                    # 订阅 Topic
                    # 修复1：添加 type 字段，部分 rosbridge 实现需要明确指定消息类型
                    ...
                    # 修复2：主动读取 subscribe 确认消息（rosbridge 会返回确认）
                    try:
                        ...
                        try:
                            ...
                            if "msg" in msg_data and "data" in msg_data["msg"]:
                                ...
                                if status == "running-started" or status == "running_started":
                                    # 判断是否有药品ID（新格式）
                                    # 注意：medicine_id 可能是整数 0，所以需要显式判断 None
                                    if medicine_id is not None and prescription_code:
                                        ...
                                        # 通知 HIS Sender 该药品已收到 running-started
                                        try:
                                ...
                                # ===== 药单完成（Step5返回）：发送end消息 =====
                                # 新逻辑：{prescription_code}_running-step5-return 表示药单完成，触发发送end
                                elif status == "running-step5-return" or status == "running_step5_return":
                                    ...
                                    if medicine_id is None and prescription_code:
                                        ...
                                        # 药品级消息：{medicine_id}_{prescription_code}_running-step5-return（旧格式，不处理）
                                ...
                                # ===== 药品完成：切换到下一个药品 =====
                                # 新逻辑：{medicine_id}_{prescription_code}_end 表示当前药品完成，触发切换药品
                                elif status == "end":
                                    print(f"[ROS Listener] 判断是否有药品ID:")
                                    print(f"[ROS Listener]   medicine_id值: {medicine_id}")
                                    print(f"[ROS Listener]   medicine_id is None: {medicine_id is None}")
                                    ...
                                    if medicine_id is not None and prescription_code:
                                        ...
                                        try:
                                            ...
                                            # 注意：current_medicine_index 是从1开始的
                                            # 判断是否是最后一个药品：current_medicine_index == medicine_total
                                        ...
                                        # 药单级end消息（不处理）
                                        print(f"[ROS Listener] 收到药单级end消息，不处理")
                                ...
                                # ===== 任务完成（药单全部完成）：停止发送 =====
                                elif status == "all_completed" or status == "task_end":
                                    logger.info(f"任务完成: {prescription_code}")
                                    print(f"[ROS Listener] 任务完成: {prescription_code}")
                                    # 通知 HIS Sender 停止发送
                                    try:
                                ...
                                # 修复4：消息格式不匹配时打印警告（此前被静默丢弃，导致无法诊断）
