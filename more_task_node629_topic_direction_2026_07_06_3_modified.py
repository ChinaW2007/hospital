...
修改版本 3：
- 支持药单级和药品级两种消息格式
- his_sub_callback 添加处理 end 消息的逻辑
- make_msg 函数支持生成两种消息格式
- Step 5 完成后发送药单级 running-step5-return
...
if __name__ == '__main__':
    ...
    #   "start"   → 药单开始，触发任务
    #   "running" → 药单运行中
    #   "end"     → HIS Sender发送的药品完成信号，回复药品级end消息
    ...
    def his_sub_callback(msg):
        ...
        if msg.data == "start":
            ...
            # ===== 新增：处理 HIS Sender 发送的 end 消息 =====
            # HIS Sender 发送 end 表示当前药品完成，准备切换下一个药品
            # ROS端收到后，发送药品级 end 消息给 HIS Sender
            rospy.loginfo(
                "Received end signal from HIS Sender, medicine_id=%d, prescription_code=%s",
                msg.medicine_id, msg.prescription_code
            )
            
            # 发送药品级 end 消息给 HIS Sender
    ...
    def make_msg(text, include_medicine_id=True):
        ...
        Args:
            text: 消息内容（如 "running-step5-return", "running-started"）
            include_medicine_id: 是否包含药品ID
                - True: 药品级消息，格式为 {medicine_id}_{prescription_code}_{text}
                - False: 药单级消息，格式为 {prescription_code}_{text}
    ...
    while not rospy.is_shutdown():
        ...
        # ===== 新逻辑：发送药单级 running-step5-return 消息 =====
        # 不包含 medicine_id，表示整个药单完成
        prescription_code = task_state.get("prescription_code", "")
        step5_return_msg = f"{prescription_code}_running-step5-return"
        pub.publish(step5_return_msg)
        rospy.loginfo(
            "Published prescription-level running-step5-return: %s",
            step5_return_msg
        )
        ...
        if goto_goal(client, HOME_X, HOME_Y, HOME_YAW):
            ...
            # ===== 注意：不发送药品级 end 消息 =====
            # HIS Sender 会发送 end 消息给 ROS
            # ROS 收到 HIS Sender 的 end 后，在 his_sub_callback 中发送药品级 end
            # 所以这里不发送，避免重复
            
            rospy.loginfo(
                "Step 5 completed, waiting for HIS Sender to send end message..."
            )
        else:
            rospy.logerr(
