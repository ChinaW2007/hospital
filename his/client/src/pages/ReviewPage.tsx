      ...
      <motion.div className="glass-card" style={{ padding: 20 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16 }}>
          <motion.div className="stat-card" whileHover={{ y: -4 }}>
            ...
            <div className="stat-value" style={{ fontSize: 28, color: '#f0ad4e' }}>{total}</div>
            <div className="stat-label" style={{ fontSize: 13 }}>待审核</div>
          </motion.div>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state"><div className="empty-icon">🎉</div><p>暂无待审核处方</p></div>
        ) : (
          <>
            <table className="glass-table" style={{ width: '100%' }}>
              <thead>
                <tr><th>处方编号</th><th>病人</th><th>诊断</th><th>医生</th><th style={{ textAlign: 'right' }}>提交时间</th><th>操作</th></tr>
              </thead>
              <tbody>
                <AnimatePresence>
